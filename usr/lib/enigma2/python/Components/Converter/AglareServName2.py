...
class AglareServName2(Converter, object):
    ...
    STREAMURL = 10
    STREAMTYPE = 11
    STREAMHOST = 12  # Added new type for stream host
    ...
    def __init__(self, type):
        ...
        elif type == "StreamHost":  # Added new type handling
    ...
    def _get_stream_type(self, refstr):
    ...
    def _extract_stream_url(self, refstr):
        ...
        try:
            ...
            if '%3a//' in refstr:
                ...
                # Decode URL encoding and replace %3a with :
                decoded_url = urllib.unquote(stream_url).decode('utf-8')
                return decoded_url.replace('%3a', ':')
            elif '://' in refstr:
                stream_url = ' '.join(refstr.split(':')[10:])
                return stream_url
        except BaseException:
            ...
            if http_index >= 0:
                return refstr[http_index:].replace('%3a', ':')
            return refstr.replace('%3a', ':')
        return ''
    ...
    def _extract_stream_host(self, refstr):
        ...
        return stream_url.split('/')[0] if '/' in stream_url else stream_url
    ...
    def getText(self):
        ...
        if self.type == self.NAME:
            ...
            if self.isStream:
                # For stream services, return only the reference part without URL
                return ':'.join(refstr.split(':')[:10])
            if self.refstr:
                return self.refstr
            return refstr
        elif self.type == self.STREAMURL:
            return self._extract_stream_url(refstr)
        elif self.type == self.STREAMTYPE:
            return self._get_stream_type(refstr)
        elif self.type == self.STREAMHOST:  # Added new type handling
            return self._extract_stream_host(refstr)
            ...
            for line in tmp:
                ...
                if f == 'N':    # %N - Name
                    ...
                    if self.isStream:
                        ...
                        ret += refstr
                elif f == 'U':  # %U - Stream URL
                    if self.isStream:
                        try:
                            if '%3a//' in refstr:
                                stream_url = ' '.join(refstr.split(':')[10:])
                                ret += urllib.unquote(stream_url).decode('utf-8')
                            elif '://' in refstr:
                                stream_url = ' '.join(refstr.split(':')[10:])
                                ret += stream_url
                        except BaseException:
                        ...
                        ret += ""
                elif f == 'T':  # %T - Stream Type
                    ret += self._get_stream_type(refstr)
