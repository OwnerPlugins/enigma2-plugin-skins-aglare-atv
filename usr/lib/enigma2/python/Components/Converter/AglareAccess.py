...
class AglareAccess(Poll, Converter):
	CAID = 0
	PID = 1
	BETACAS = 2
	IRDCAS = 3
...
# OE-A
		if exists("/etc/image-version") and not exists("/etc/.emustart") and not exists("/etc/CurrentBhCamName") and not exists("/etc/EGCamConf") and not exists("/etc/clist.list"):
...
# OpenVix_script
		if exists("/etc/image-version") and exists("/usr/lib/enigma2/python/Plugins/SystemPlugins/ViX/SoftcamManager.pyc"):
			for line in open("/etc/image-version"):
				if "OpenViX" in line:
					try:
						if config.softcam.actCam.value:
...
# Others
		if serlist is not None:
			...
			cardserver = "N/A"
		print('Cardserver is:', cardserver)
	...
	def ecmfile(self):
		...
		if service:
			try:
				ecm_mtime = stat(ecmpath).st_mtime
				if not stat(ecmpath).st_size > 0:
					info = {}
