...
class AglareCaidInfo2(Poll, Converter, object):
	CAID = 0
	PID = 1
	PROV = 2
	ALL = 3
	...
	def __init__(self, type):
		...
		if DBG:
			j00zekDEBUG('[j00zekModCaidInfo2:__init__] self.NUM_CI = %s' % str(self.NUM_CI))
		if type == "CAID":
			self.type = self.CAID
		elif type == "PID":
			self.type = self.PID
	...
	def runningSoftCamName(self, fullName=False):
		if DBG:
			j00zekDEBUG('[j00zekModCaidInfo2:runningSoftCamName] >>> fullName="%s"' % str(fullName))
		...
		if self.NUM_CI and self.NUM_CI > 0:
			if self.eDVBCIUIInstance:
				for slot in range(self.NUM_CI):
					...
					if DBG:
						j00zekDEBUG('\t slot=%s, state=%s' % (slot, state))
				...
				if CIstring:
					...
					if DBG:
						j00zekDEBUG('\t CIstring="%s"' % CIstring)
			return CIstring

	@cached
	def getText(self):
		if DBG:
			j00zekDEBUG('[j00zekModCaidInfo2:getText] >>>self.type="%s", ciFormat.value="%s"' % (self.type, config.plugins.j00zekCC.ciFormat.value))
		textvalue = ""
		server = ""
		softCamName = ''
		service = self.source.service
	...
	def ecmfileContent(self):
		if DBG:
			j00zekDEBUG('[j00zekModCaidInfo2:ecmfileContent] >>>')
		self.poll_interval = self.my_interval
		self.poll_enabled = True
		ecminfo = ""
		try:
			...
			if DBG:
				j00zekDEBUG('\t Exception analyzing content of /tmp/ecm.info : %s' % str(e))
		return ecminfo
	...
	def ciModuleStateChanged(self, slot):
		if DBG:
			j00zekDEBUG('[j00zekModCaidInfo2:getText] >>>')
		self.changed(True)
